...
namespace supportsapi.labgenomics.com.Controllers.Sales
{
    ...
    public class CompRegistOrderController : ApiController
    {
        ...
        {
            JObject request = JObject.Parse(param);
            try
            {
                JArray arrayResponse = new JArray();
                //비즈먼트
                if (request["TransKind"].ToString() == "Bizment")
                {
                    Bizment bizment = new Bizment();
                    arrayResponse = bizment.GetOrder(request);
                }
                //브레인
                else if (request["TransKind"].ToString() == "Brain")
                {
                    Brain brain = new Brain();
                    arrayResponse = brain.GetOrder(request);
                }
                //이지스
                else if (request["TransKind"].ToString() == "Eghis")
                {
                    Eghis eghis = new Eghis();
                    arrayResponse = eghis.GetOrder(request);
                }
                //유비케어 Cloud EMR
                else if (request["TransKind"].ToString() == "CloudEMR")
                {
                    CloudEMR cloudEMR = new CloudEMR();
                    arrayResponse = cloudEMR.GetOrder(request);
                }
                //유비케어 의사랑2000
                else if (request["TransKind"].ToString() == "Ysr2000")
                {
                    Ysr2000 ysr2000 = new Ysr2000();
                    arrayResponse = ysr2000.GetOrder(request);
                }
                //엑셀
                else if (request["TransKind"].ToString() == "Excel")
                {
                    ExcelOrder excelOrder = new ExcelOrder();
                    arrayResponse = excelOrder.GetOrder(request);
                }
                //PGS사업용
                else if (request["TransKind"].ToString() == "PGS")
                {
                    PGS pgs = new PGS();
                    arrayResponse = pgs.GetOrder(request);
                }
                //통합 API
                else
                {
                    IntegratedAPI integratedAPI = new IntegratedAPI();
                    arrayResponse = integratedAPI.GetOrder(request);
                }
