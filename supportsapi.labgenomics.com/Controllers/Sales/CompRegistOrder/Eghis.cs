                ...
                if (request["RegKind"].ToString() == "W") //등록대기
                {
                    ...
                    foreach (DataRow row in dtOrder.Rows)
                    {
                        sql += $"MERGE INTO RsltTransEghisOrder AS target\r\n" +
                               $"USING (SELECT '{row["hosp_no"]}' AS hosp_no, '{row["CompCode"]}' AS CompCode, '{row["clinic_ymd"]}' AS clinic_ymd, '{row["ord_ymd"]}' AS ord_ymd\r\n" +
                               ...
                               $"       , acc_ymd, acc_time, sutak_sts, edi_cd, dept_cd, dept_nm, doct_nm, health_gb)\r\n" +
                               $"VALUES ( '{row["hosp_no"]}', '{row["hosp_nm"]}', '{row["CompCode"]}', '{row["clinic_ymd"]}', '{row["ord_ymd"]}', '{row["recept_no"]}', '{row["ord_cd"]}'\r\n" +
