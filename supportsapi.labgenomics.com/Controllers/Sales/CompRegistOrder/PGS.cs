            ...
            try
            {
                ...
                if (request["RegKind"].ToString() == "W")
                {
                    sql = 
                        ...
                        $"  , pcsc.CompSubCode, pcsc.CompSubName\r\n" +
                        ...
                        $"JOIN PGSCustomerEmployee pce\r\n" +
                        //$"ON pce.EmpCode = ppi.CustomerEmpCode\r\n" +
                        $"ON pce.EmpCode = CASE WHEN pcc.CompName like '%교보%' THEN ppi.CompCode ELSE ppi.CustomerEmpCode END\r\n" +       //박상준 수정(2021-10-22)
                                                                                                                                          //$"JOIN ProgCompSubCode pcsc\r\n" +
                        $"LEFT OUTER JOIN ProgCompSubCode pcsc\r\n" + //박상준 수정(2021-10-22)
                        $"ON pcsc.CompCode = ppi.CompCode\r\n" +
                        ...
                        $"WHERE ppi.ReservationDateTime >= '{request["BeginDate"].ToString()}'\r\n" +
                        $"AND ppi.ReservationDateTime < DATEADD(DAY, 1, '{request["EndDate"].ToString()}')\r\n" +
